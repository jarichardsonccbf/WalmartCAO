...
wk17 <- rbind(wk17pack, java.mon, mon.rehab, single.g.peak) %>% 
...
write.csv(wk17.instocks, "outputs/wk17.cases.csv")
...
write.csv(no.carry, "outputs/CCBF.wk17.no.carry.csv")
# manual inspection of previous sales data reveals these items are not carried by these stores

# Calculate errors
source("functions/functions.R")

CalculateErrors(wk17.instocks)
CalculateErrors(wk17.ccbf.instocks)

# Calculate errors for products
product_metrics17 <- wk17.ccbf.instocks %>% 
...
write.csv(product_metrics17 %>% 
            select(PRODUCT, count, ME, MAE), "outputs/wk17.cases.product.error.csv")
