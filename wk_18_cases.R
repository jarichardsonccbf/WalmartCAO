...
wk18 <- rbind(wk18pack, java.mon, mon.rehab, single.g.peak) %>% 
...
write.csv(wk18.instocks, "outputs/wk18.cases.csv")
...
write.csv(no.carry, "outputs/CCBF.wk18.no.carry.csv")
# manual inspection of previous sales data reveals these items are not carried by these stores

# Calculate errors
source("functions/functions.R")

CalculateErrors(wk18.instocks)
CalculateErrors(wk18.ccbf.instocks)

# Calculate errors for products
product_metrics18 <- wk18.ccbf.instocks %>% 
...
write.csv(product_metrics18 %>% 
            select(PRODUCT, count, ME, MAE), "outputs/wk18.cases.product.error.csv")
