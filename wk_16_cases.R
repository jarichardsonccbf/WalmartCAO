...
wk16 <- rbind(wk16pack, java.mon, mon.rehab, single.g.peak) %>% 
...
write.csv(wk16.instocks, "outputs/wk16.cases.csv")
...
write.csv(no.carry, "outputs/CCBF.wk16.no.carry.csv")
# manual inspection of previous sales data reveals these items are not carried by these stores

# Calculate errors
source("functions/functions.R")

CalculateErrors(wk16.instocks)
CalculateErrors(wk16.ccbf.instocks)

# Calculate errors for products
product_metrics16 <- wk16.ccbf.instocks %>% 
...
write.csv(product_metrics16 %>% 
            select(PRODUCT, count, ME, MAE), "outputs/wk16.cases.product.error.csv")
