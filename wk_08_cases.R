...
wk08 <- rbind(wk08pack, java.mon, mon.rehab, single.g.peak) %>% 
...
write.csv(wk08.instocks, "outputs/wk08.cases.csv")
...
write.csv(no.carry, "outputs/CCBF.wk08.no.carry.csv")
#manual inspection of previous sales data reveals these items are not carried by these stores

#Calculate errors
source("functions/functions.R")

CalculateErrors(wk08.instocks)
CalculateErrors(wk08.ccbf.instocks)

#Calculate errors for products
product_metrics8 <- wk08.ccbf.instocks %>% 
...
write.csv(product_metrics8 %>% 
            select(PRODUCT, count, ME, MAE), "outputs/wk08.cases.product.error.csv")